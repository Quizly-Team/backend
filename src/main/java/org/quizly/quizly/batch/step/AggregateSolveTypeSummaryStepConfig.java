...
import org.quizly.quizly.core.domain.entity.SolveTypeSummary;
...
import org.quizly.quizly.core.domain.repository.SolveHistoryStatisticsRepository.QuizTypeSummary;
        ...
        return user -> {
            try {
                ...
                List<QuizTypeSummary> typeSummaryList =
                    solveHistoryStatisticsRepository.findFirstAttemptsByQuizTypeAndDate(user,
                        targetDate);
                ...
                for (QuizTypeSummary typeSummary : typeSummaryList) {
                    ...
                    solveTypeSummary.setSolvedCount(
                        Optional.ofNullable(typeSummary.getTotalCount()).map(Long::intValue)
                            .orElse(0));
                    solveTypeSummary.setCorrectCount(
                        Optional.ofNullable(typeSummary.getCorrectCount()).map(Long::intValue)
                            .orElse(0));
                ...
                log.error(
                    "[AggregateSolveTypeSummaryProcessor] Failed for user: {}, targetDate: {}",
