...
public class CreateTopicOpenAiService implements BaseService<CreateTopicOpenAiRequest, CreateTopicOpenAiResponse> {
  ...
  public CreateTopicOpenAiResponse execute(CreateTopicOpenAiRequest request) {
    ...
    try (Response response = createRequest(httpRequest)) {
      ...
      if (body == null) {
        log.error("[CreateTopicOpenAiService] response body is null. response code: {}", response.code());
        return CreateTopicOpenAiResponse.builder()
            .success(false)
      ...
      if (!response.isSuccessful()) {
        log.error("[CreateTopicOpenAiService] OpenAi API returned non-successful code: {}, body: {}", response.code(), responseBody);
        return CreateTopicOpenAiResponse.builder()
            .success(false)
      ...
      try {
        ...
        if (content == null || content.isBlank()) {
          log.error("[CreateTopicOpenAiService] Empty content in OpenAi response. Body: {}", responseBody);
          return CreateTopicOpenAiResponse.builder()
              .success(false)
              .errorCode(OpenAiErrorCode.EMPTY_OPENAI_RESPONSE_BODY)
              .build();
        ...
        GeneratedTopicResponse generatedTopicResponse = objectMapper.readValue(content, GeneratedTopicResponse.class);

        if (generatedTopicResponse == null || generatedTopicResponse.getTopic() == null) {
          log.error("[CreateTopicOpenAiService] Failed to parse topic from content: {}", content);
          return CreateTopicOpenAiResponse.builder()
              .success(false)
        ...
        log.error("[CreateTopicOpenAiService] Failed to parse OpenAi API response. Body: {}", responseBody, e);
        return CreateTopicOpenAiResponse.builder()
            .success(false)
      ...
      log.error("[CreateTopicOpenAiService] OpenAi API request failed due to network I/O error.", e);
      return CreateTopicOpenAiResponse.builder()
          .success(false)
