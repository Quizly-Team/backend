...
public class ReadQuizTypeSummaryService implements
    BaseService<ReadQuizTypeSummaryService.ReadQuizTypeSummaryRequest, ReadQuizTypeSummaryService.ReadQuizTypeSummaryResponse> {
    ...
    private Map<QuizType, QuizTypeCounts> getTodaySummaryMap(User user, LocalDate today) {
        ...
        solveHistoryStatisticsRepository
            ...
            .forEach(summary -> {
                int solved = Optional.ofNullable(summary.getTotalCount()).map(Long::intValue)
                    .orElse(0);
                int correct = Optional.ofNullable(summary.getCorrectCount()).map(Long::intValue)
                    .orElse(0);
    ...
    @AllArgsConstructor
    @ToString
    public static class ReadQuizTypeSummaryResponse extends
        BaseResponse<ReadQuizTypeSummaryErrorCode> {
