...
public class ReadHourlySummaryService implements
    BaseService<ReadHourlySummaryService.ReadHourlySummaryRequest, ReadHourlySummaryService.ReadHourlySummaryResponse> {
    ...
    public ReadHourlySummaryResponse execute(ReadHourlySummaryRequest request) {
        ...
        List<ReadHourlySummaryResponse.HourlySummary> hourlySummaryList = readHourlySummaryList(
            user);
    ...
    private List<ReadHourlySummaryResponse.HourlySummary> readHourlySummaryList(User user) {
        ...
        return Arrays.stream(TIME_SLOTS)
            .mapToObj(
                slot -> new ReadHourlySummaryResponse.HourlySummary(slot, hourlyCountMap.get(slot)))
            .toList();
    }

    private Map<Integer, Integer> getPastHourlyDataMap(User user, LocalDate startDate,
        LocalDate endDate) {
    ...
    private Map<Integer, Integer> getTodayHourlyDataMap(User user, LocalDate today) {
        ...
        List<SolveHistoryStatisticsRepository.HourlySummary> todayHourlyData =
            solveHistoryStatisticsRepository.findFirstAttemptsHourlySummaryByUserAndDate(user,
                today);
